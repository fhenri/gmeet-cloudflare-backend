...
const initGoogleCalendar = async (env: {
	...
	try {
		...
		const auth = await getGoogleAuthToken(credentials.client_email, credentials.private_key, OAUTH_SCOPES);
...
const buildDateSlots = async (date: Date) => {
	const dateSlots = availableSlots.map((slot) => {
		const cetDateTime = new Date(date.getFullYear(), date.getMonth(), date.getDate(), +slot.slice(0, 2), +slot.slice(3, 5));
